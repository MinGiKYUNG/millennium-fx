...
int total_distance(void) //total distance 
{
        /*
        time_t ltime;
        struct tm *cur_time;
...
int check_distance(int ch) //Max : 555, Min : 5
{
	...
	sensor_value_default = total / 30;
	printf("sensor before = %d\n",sensor_value_default);
	return sensor_value_default;
	
	/*
	sensor_value_default = 560 - (total / 100);

	printf("sensor-%d => %d\n",ch,sensor_value_default);

	//transform default to mm
	sensor_value_mm = (int)((sensor_value_default-4)*(0.6)) + 30;	
	//	

	printf("transformed sensor-%d => %d\n\n",ch,sensor_value_mm);
	
	usleep(100000);	

	return sensor_value_mm;
	...
	sensor[0] = 340 - ((int)rxbuf[0] * 0.5);
	sensor[1] = 340 - ((int)rxbuf[1] * 0.5);
	sensor[2] = 340 - ((int)rxbuf[2] * 0.5);
	sensor[3] = 340 - ((int)rxbuf[3] * 0.5);

	
	printf("ch1 %04d mm,  ch2 %04d mm,  ch3 %04d mm,  ch4 %04d mm\n",
