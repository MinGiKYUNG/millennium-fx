...
/*
/////////////9월 8일 10시52분 기계에서 받은값을 흑백으로 변환//
void deb_buf_con_to_gray(struct videobuf_dev* simple)
{
		IplImage *img = cvCreateImage(cvSize(320, 240), 8, 1);
		IplImage *temp_img = cvCreateImage(cvSize(320, 240), 8, 1);	
		...
		img->colorModel[4] = NULL;
		img->channelSeq[4] = NULL;
		...
		img->imageId = NULL;
		img->tileInfo= NULL;
		img->imageSize=320*240;
		img->imageData=(char*)(simple->buf);
		img->widthStep=320;
		img->BorderMode[4]=NULL;
		img->borderConst[4]=NULL;
		img->imageDataOrigin=NULL;
		
		cvCvtColor(img, temp_img, CV_BGR2GRAY);
		
		simple->buf = temp_img->imageData;
}
*/
